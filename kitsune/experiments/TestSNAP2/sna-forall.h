...
#include "Kokkos_forall_replacements.h"
   ...
   public:
    ...
    void operator()(const ComputeUiTag, const member_type team_member) const {
        ...
        if (iter < nTotal) {
            ...
            atomicAdd(&(ulisttot(natom, 0).re),
                               sfac * ulist(natom, nbor, 0).re);
            atomicAdd(&(ulisttot(natom, 0).im),
                               sfac * ulist(natom, nbor, 0).im);
            ...
            for (int j = 1; j <= twojmax; j++) {
                ...
                for (int m_iter = 0; m_iter < m_max; ++m_iter) {
                    ...
                    atomicAdd(&(ulisttot(natom, jju).re),
                                       sfac * ulist(natom, nbor, jju).re);
                    atomicAdd(&(ulisttot(natom, jju).im),
                                       sfac * ulist(natom, nbor, jju).im);
                    jju++;
                }
    ...
    void operator()(const ComputeYiTag, const int iter) const {
        ...
        atomicAdd(&(ylist(natom, jjdu).re), betaj * ztmp_r);
...
#pragma unroll
        ...
        //                    betaj * ztmp_i);
        atomicAdd(&(ylist_re_gpu(natom_mod, jju_half, natom_div)),
                           betaj * ztmp_r);
        atomicAdd(&(ylist_im_gpu(natom_mod, jju_half, natom_div)),
                           betaj * ztmp_i);
    }
