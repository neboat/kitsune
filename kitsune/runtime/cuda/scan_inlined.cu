...
__attribute__((always_inline)) inline __device__ void
    ...
    if (warp_id == 0) {
        ...
        if (lane_id == KITCUDA_WARP_SIZE - 1) {
            ...
            for (size_t lookback_index = blockIdx.x; lookback_index--;) {
                ...
                while ((state = scan_state[lookback_index]) ==
                       KITCUDA_SCAN_STATE_X)
                    __threadfence(); // Wait for the state to change
                int32_t *src =
                    (state == KITCUDA_SCAN_STATE_A ? aggregate
                                                   : inclusive_prefix) +
        ...
        for (size_t i = 0; i < size / sizeof(int32_t); i++)
            temp_1[i] =
                __shfl_sync(0xFFFFFFFF, temp_1[i], KITCUDA_WARP_SIZE - 1);
        __syncwarp();
        if (lane_id < n_warps) {
            size_t idx = 0;
...
__global__ void kernel_test(size_t n, range *aggregate, range *inclusive_prefix,
    ...
    __kitcuda_scan(reinterpret_cast<int32_t *>(&view),
                   reinterpret_cast<int32_t *>(&temp_1),
                   reinterpret_cast<int32_t *>(&temp_2),
                   reinterpret_cast<int32_t *>(&temp_3),
                   reinterpret_cast<int32_t *>(shmem),
                   reinterpret_cast<int32_t *>(aggregate),
                   reinterpret_cast<int32_t *>(inclusive_prefix), scan_state,
                   sizeof(range), id_range, reduce_range);
    if (idx < n) {
